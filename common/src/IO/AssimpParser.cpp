...
void AssimpParser::processMaterials(const aiScene& scene, Logger& logger)
{
  ...
  for (unsigned int i = 0; i < scene.mNumMaterials; i++)
  {
    ...
    // RB: valid material names in the .mtr files are the interface between Blender,
    // TrenchBroom and the engine
    ...
    {
      material =
        IO::loadShader(Path(Model::BrushFaceAttributes::NoTextureName), m_fs, logger);
      m_textures.push_back(std::move(*material));
    }
  }
#else
