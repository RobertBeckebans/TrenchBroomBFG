...
std::ostream& operator<<(std::ostream& str, const ObjSerializer::BrushObject& object)
{
  ...
  {
    // RB: add textures/ for Doom 3 because the material name is the interface for the OBJ
    // loader
...
static void writeVertices(std::ostream& str, const std::vector<vm::vec3>& vertices)
{
  ...
  {
    // RB: use raw X, Y, Z without scaling or flipping for Doom 3 so we can reuse the .obj
