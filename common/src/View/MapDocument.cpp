...
void MapDocument::removeNodes(const std::vector<Model::Node*>& nodes)
{
  ...
  const auto changedLinkedGroups =
    findAllLinkedGroups(*m_world, kdl::map_keys(removableNodes));
  ...
  const Assets::BrushEntityDefinition* definition)
{
  ...
  // RB: add unique entity name right from the beginning and also avoid a missing "model"
  // key
  std::string uniqueName;
  m_world->generateUniqueTargetnameForClassname(definition->name(), uniqueName);
  ...
  m_world->accept(kdl::overload(
    ...
    [=](Model::EntityNode* entityNode) {
      ...
      if (entityNode->hasMissingTargetname() || entityNode->hasConflictingTargetname())
      {
        ...
        // second if this is a brush entity then Doom 3 can't load the model if there is
        // no "model" key so make sure it has one and it's the same as the name
        if (
...
void MapDocument::registerIssueGenerators()
{
  ...
  // RB: LinkSourceIssueGenerator would be a bug with Doom 3 because entities are supposed
  // to have unique names even there is no other entity targeting them
  ...
  m_world->registerIssueGenerator(
    new Model::SoftMapBoundsIssueGenerator(m_game, m_world.get()));
  ...
  m_world->registerIssueGenerator(
    new Model::PropertyKeyWithDoubleQuotationMarksIssueGenerator());
  m_world->registerIssueGenerator(
    new Model::PropertyValueWithDoubleQuotationMarksIssueGenerator());
